...
export const UpdateLineItemModal: React.FC<UpdateLineItemModalProps> = ({
  ...
  return (
    <Modal
      isOpen={isOpen}
      onClose={handleClose}
      title={t("purchasing.updateLineItem.title")}
    >
      <div className="space-y-6">
        ...
        <div className="bg-slate-50 p-4 rounded-lg border">
          <div className="flex items-center gap-2 text-slate-700 font-semibold mb-2">
            ...
            {t("purchasing.updateLineItem.productDetails")}
          </div>
          <div className="space-y-1">
            <div className="font-medium">
              {lineItem.inventoryId?.productName ||
                t("purchasing.updateLineItem.unknownProduct")}
            </div>
            <div className="text-sm text-slate-600">
              {t("purchasing.updateLineItem.code")} {lineItem.inventoryId?.productCode || "-"} | {t("purchasing.updateLineItem.sku")} {lineItem.inventoryId?.SKU || "-"}
            </div>
            <div className="text-sm text-slate-600">
              {t("purchasing.updateLineItem.unitPrice")} {lineItem.unitPrice.toLocaleString()} | {t("purchasing.updateLineItem.total")} {lineItem.totalPrice.toLocaleString()}
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="bg-blue-50 p-4 rounded-lg border border-blue-200">
            <label className="block text-sm font-medium text-blue-700 mb-2">
              {t("purchasing.updateLineItem.receivedQuantity")}
            </label>
          ...
          <div className="bg-green-50 p-4 rounded-lg border border-green-200">
            <label className="block text-sm font-medium text-green-700 mb-2">
              {t("purchasing.updateLineItem.goodQuantity")}
            </label>
            <input
              type="number"
              min="0"
          ...
          <div className="bg-red-50 p-4 rounded-lg border border-red-200">
            <label className="block text-sm font-medium text-red-700 mb-2">
              {t("purchasing.updateLineItem.badQuantity")}
            </label>
            <input
              type="number"
              min="0"
        ...
        <div>
          <label className="block text-sm font-medium text-slate-700 mb-2">
            {t("purchasing.updateLineItem.expiryDate")}
          </label>
          <input
            type="date"
            value={expiryDate}
        ...
        <div className="bg-amber-50 p-3 rounded-lg border border-amber-200 space-y-1">
          <div className="text-sm text-amber-800">
            <strong>{t("purchasing.updateLineItem.noteWarning")} </strong>
            {t("purchasing.updateLineItem.qtySumRule")} ({lineItem.receivedQuantity}). {t("purchasing.updateLineItem.currentTotal")} {goodQuantity + badQuantity}
          </div>
          {lineItem.transferredQuantity !== undefined && lineItem.transferredQuantity > 0 && (
            <div className="text-xs text-amber-700">
              * {lineItem.transferredQuantity} {t("purchasing.updateLineItem.alreadyTransferredNotice")} {lineItem.transferredQuantity}.
            </div>
          )}
        </div>
        ...
        <div className="flex justify-end gap-3 pt-4 border-t">
          <button
            onClick={handleClose}
            disabled={updating}
            className="px-4 py-2 text-slate-600 bg-slate-100 hover:bg-slate-200 rounded-md transition-colors disabled:opacity-50 cursor-pointer"
          >
            {t("purchasing.updateLineItem.cancel")}
          </button>
          <button
            onClick={handleSave}
            disabled={
            ...
            className="px-4 py-2 text-white bg-green-600 hover:bg-green-700 rounded-md transition-colors disabled:opacity-50 flex items-center gap-2 cursor-pointer"
          >
            {updating ? (
              <>
                <div className="animate-spin rounded-full h-4 w-4 border-b-2 border-white"></div>
                {t("purchasing.updateLineItem.updating")}
              </>
            ) : (
              <>
                <Save className="w-4 h-4" />
                {t("purchasing.updateLineItem.updateLineItem")}
              </>
            )}
          </button>
        </div>
